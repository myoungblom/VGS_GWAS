...
# Note: path to vcflib executable will need to be changed

# Format for traits file:
#	- isolates in first column (no header)
#	- other columns are traits (with headers)
# 	- use '0' and '1' to designate positive/negative for each isolate/trait
#eg.
# ,trait1,trait2
#iso1,0,1
#iso2,1,0
...
if len(sys.argv) != 4:
    print("Usage: scoaryToVcflibFst.py <traits.csv> <snpvcf.vcf> <trait>")
    sys.exit(0)

traitFile = sys.argv[1]
vcfFile = sys.argv[2]
trait = str(sys.argv[3])
